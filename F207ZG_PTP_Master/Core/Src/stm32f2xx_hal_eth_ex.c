...
#include "stm32f2xx_hal_eth_ex.h"
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_ETH_TransmitFrame_TimeStamp(ETH_HandleTypeDef *heth,
		uint32_t FrameLength, ETH_TimeStamp *TimeStamp)
{
	...
	if (bufcount == 1U)
	{
		...
		heth->TxDesc->Status |= ETH_DMATXDESC_TTSE | ETH_DMATXDESC_FS
				| ETH_DMATXDESC_LS;
		/* Set frame size */
	...
	{
		timeout++;
	} while (!(heth->TxDesc->Status & ETH_DMATXDESC_TTSS)
			&& (timeout < PHY_READ_TO));
...
void ETH_PTPTimeStampCmd(FunctionalState NewState)
{
	...
	if (NewState != DISABLE)
	{
		...
		ETH->PTPTSCR |= ETH_PTPTSCR_TSE | ETH_PTPTSSR_TSSIPV4FE
				| ETH_PTPTSSR_TSSIPV6FE | ETH_PTPTSSR_TSSARFE;
	}
	else
	{
